...
*/
	if (q == NULL){ //kdyz mame prazdny zasobnik volame funkci queueError(QERR_INIT).
        queueError(QERR_INIT);
	}
	else{
	int i;
    for (i = 0; i<QUEUE_SIZE; i++)
        q->arr[i] = '*'; // všechny hodnoty v poli q->arr nastaveme na '*'
    q->f_index = 0;//index na začátek fronty nastaveme na 0
    q->b_index = 0;//index prvního volného místa nastavte také na 0
	}
...
*/
	return (index + 1) % QUEUE_SIZE;//vrací index následujícího prvku v poli
...
*/
	return (q->f_index==q->b_index)? 1 : 0;//Vrací nenulovou hodnotu, pokud je frona prázdná, jinak vrací hodnotu 0
	  //solved = FALSE;                  /* V případě řešení, smažte tento řádek! */
}
...
*/
	  ...
	  return (q->f_index == nextIndex(q->b_index))? 1: 0;//rací nenulovou hodnotu, je-li fronta plná, jinak vrací hodnotu 0
//solved = FALSE;                  /* V případě řešení, smažte tento řádek! */
}
...
*/
	...
	if (queueEmpty(q))//Pokud je fronta prázdná, ošetřete to voláním funkce queueError(QERR_FRONT)
	{
    queueError(QERR_FRONT);
    }
    else{
    *c=q->arr[q->f_index];//Prostřednictvím parametru c vrátí znak ze začátku fronty q.
    }
 // solved = FALSE;                  /* V případě řešení, smažte tento řádek! */
}
...
*/
	...
	if (queueEmpty(q)){//Pokud je fronta prázdná, ošetřeme vzniklou chybu voláním funkce queueError(QERR_REMOVE).
        queueError(QERR_REMOVE);
    }
    else{
    q->f_index = nextIndex(q->f_index);//Odstraní znak ze začátku fronty q
    }
	  //solved = FALSE;                  /* V případě řešení, smažte tento řádek! */
}
...
*/
	...
	if(queueEmpty(q)){//Pokud je fronta prázdná, ošetřete to voláním funkce queueError(QERR_GET)
		queueError(QERR_GET);
	}
	else{
	queueFront(q, c);//precte hodnoty prvniho prvku z fronty
	queueRemove(q);//Odstraní prvni znak ze začátku fronty
	}
	 // solved = FALSE;                  /* V případě řešení, smažte tento řádek! */
}
...
*/
    ...
    q->arr[q->b_index] = c;//Vloží znak c do fronty.Pokud je fronta plná, ošetřeme chybu voláním funkce queueError(QERR_UP). 
